            ...
            if self.mode == "Адаптивний":
                ...
                if not results:
                    ...
                    for r in reversed(results):  # Останні зверху
                        tree.insert("", "end",
                                    values=(r["test"], r["user"], r["mode"], f"{r['score']}/{r['max_score']}",
                                            r["duration"], r["date"]))
                    ...
                    tk.Label(self, text=f"Загальний середній бал по всій системі: {avg:.2f}",
                             font=("Arial", 12, "bold"), fg="#28a745").pack(pady=5)

                tk.Button(self, text="Повернутися до меню", font=("Arial", 12), command=app.show_main_menu).pack(
                    pady=10)
